...
global.authFile = `RobleBOTSession`
...
if (opts['autocleartmp'] && (global.support || {}).find) (tmp = [os.tmpdir(), 'tmp', "RobleJadiBot"], tmp.forEach(filename => cp.spawn('find', [filename, '-amin', '2', '-type', 'f', '-delete'])))}, 30 * 1000)}
opts['server']=true;
...
if (reason == 405) {
await fs.unlinkSync("./RobleBOTSession/" + "creds.json")
...
let prekey = []
let directorio = readdirSync("./RobleBOTSession")
let filesFolderPreKeys = directorio.filter(file => {
return file.startsWith('pre-key-')
})
prekey = [...prekey, ...filesFolderPreKeys]
filesFolderPreKeys.forEach(files => {
unlinkSync(`./RobleBOTSession/${files}`)
})
} 

function purgeSessionSB() {
try {
const listaDirectorios = readdirSync('./RobleJadiBot/');
let SBprekey = [];
listaDirectorios.forEach(directorio => {
if (statSync(`./RobleJadiBot/${directorio}`).isDirectory()) {
const DSBPreKeys = readdirSync(`./RobleJadiBot/${directorio}`).filter(fileInDir => {
return fileInDir.startsWith('pre-key-')
})
SBprekey = [...SBprekey, ...DSBPreKeys];
DSBPreKeys.forEach(fileInDir => {
if (fileInDir !== 'creds.json') {
unlinkSync(`./RobleJadiBot/${directorio}/${fileInDir}`)
}})
}})
...
const directories = ['./RobleBOTSession/', './RobleJadiBot/']
