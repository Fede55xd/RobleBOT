...
const handler = async (m, {conn, args: [effect], text: txt, usedPrefix, command, name}) => {
  if (!effect) throw `*Hola, use el comando de esta forma:*\n*${usedPrefix + command} efecto texto*\n*EJEMPLO*\n*${usedPrefix + command} American-flag-3D ROBLEBOT*\n\n*Si le dice que le falta texto entonces use:*\n*${usedPrefix + command} efecto texto1|texto2*\n*EJEMPLO*\n*${usedPrefix + command} Wolf-Logo-Galaxy RobleBOT|RobleUY*\n\n*✨ 𝙇𝙄𝙎𝙏𝘼 𝘿𝙀 𝙇𝙊𝙂𝙊𝙎*\n\n🤍 ${usedPrefix + command} ` + effects.map(v => v.title).join(`\n💜 ${usedPrefix + command} `)
 ...
 try {
  ...
  const res = await maker(effectoSelect?.url, [...text]).catch(_ => { throw `*Falta el.texto en el que se creara el logo.*` })
   if (typeof res == 'number') throw res == -1 ? `*El logo ${effect} no existe.*` : `*Use el comando de esta forma:*\n\n${usedPrefix + command} ${effect} ${new Array(res).fill('texto').map((v, i) => v + (i ? i + 1 : '')).join('|')}*`
