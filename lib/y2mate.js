...
async function yt144p(url, type, server = 'en68') {
  ...
  const filesize = list['144p'] || list['240p'] || list['360p']; // Choose the lowest available quality (starting from 144p)
  ...
  const res2 = await post(`https://www.y2mate.com/mates/${server}/convert`, {
    ...
    token: '',
    ftype: type,
    fquality: '144p', // Set the desired quality to 144p
  });
